 ...
 *
 * Session shape (in app code): { id, task, taskId?, ms, endedAt }
 *   - id:      string (Firestore doc id, or millisecond timestamp for local)
 *   - task:    string  (denormalized task name for display)
 *   - taskId:  string? (reference to tasks collection — null for legacy sessions)
...
const firestoreSessionsBackend = {
  ...
  async add(userId, session) {
    ...
    const ref = await addDoc(sessionsCol(db, userId), docData);
